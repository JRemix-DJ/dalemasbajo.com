...
class Products extends CI_Controller {
    ...
    private function _upload_featured_image($field = 'featured_image'){
        ...
        $dir = FCPATH.'assets/uploads/covers/';
    ...
    public function edit_product(){
        ...
        if(!empty($_FILES['demo']['name'])){
            $demo_folder = 'assets/uploads/demos/';
        ...
        if(!empty($_FILES['descargable']['name'])){
            $file_folder = 'assets/uploads/descargables/';
	...
	public function user_pay_for_it($user_id, $product_id){
		return (bool)$this->orders_model->user_files($user_id, $product_id);
	}
	...
	public function descargar_producto(){
		if($this->session->userdata('is_logued_in')){
			...
			$product_id = $this->uri->segment(3);
			$tokenstotal = ($tokens == false) ? 0 : $tokens[0]->total;
